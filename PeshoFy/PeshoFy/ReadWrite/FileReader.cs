...
namespace PeshoFy.Classes
{
    class FileReader
    {
        ...
        public void ReadFile()
        {
            ...
            foreach (string line in File.ReadLines(Constants.FILE_PATH))
            {
                if (line != null)
                {
                    ...
                    {
                        GetListenerInfo(listenerRegex, line);
                    }
                    else if (artistRegex.IsMatch(line))
                    {
                        GetArtistInfo(artistRegex, line);
                    }
                    else if (albumRegex.IsMatch(line))
                    {
                        GetAlbumInfo(albumRegex, line);
                    }
                    else if (songRegex.IsMatch(line))
                    {
                        GetSongInfo(songRegex, line);
                    }
                    else if (playlistRegex.IsMatch(line))
                    {
                        GetPlaylistInfo(playlistRegex, line);
                    }
                }
            }
        }

        public void GetListenerInfo(Regex listenerRegex, string line)
        {
            ...
            foreach (string playlistName in playListsNames)
            {
                if (playlistName != "")
                {
                    ...
                    {
                        Storage.Playlists[playlistName] = playlistToAdd;
                    }
                }
            }
            ...
            Storage.UserTypes.Add(username, "listener");
        }

        private void GetArtistInfo(Regex artistRegex, string line)
        {
            ...
            Storage.UserTypes.Add(username, "artist");
        }

        private void GetAlbumInfo(Regex albumRegex, string line)
        {
        ...
        private void GetSongInfo(Regex songRegex, string line)
        {
        ...
        private void GetPlaylistInfo(Regex playlistRegex, string line)
        {
